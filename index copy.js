...
function calculateElo(gameData) {
    ...
    gameData = gameData.map(player => {
        let baseChange = player.teamWin ? 15 : -15;
        let teamAvgElo = player.role === "mafia" ? mafiaAvgElo : citizenAvgElo;

        let eloChange;
        if (player.teamWin) {
            // Winning team: Elo gain decreases with place
            eloChange = baseChange - (player.place - 1);
        } else {
            // Losing team: Elo loss decreases with higher place (less negative)
            eloChange = baseChange + (player.place - 1);
        }

        // Adjust for adaptive change based on player's old Elo and team average Elo
        const adaptiveChange = eloChange * (1 - ((player.oldElo - teamAvgElo) / (player.oldElo + teamAvgElo)));
